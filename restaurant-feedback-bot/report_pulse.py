...
async def build_reports_for_manager(
    ...
    *,
    ...
    if not current and not (dept == chef_survey.DEPARTMENT_ALL and current_raw):
        ...
        return [f"За период «{escape(plabel)}» ответов пока нет.\n\n{hint}"]
...
async def build_calendar_month_reports(
    ...
    *,
    ...
    """Подробный отчёт за календарный месяц. Возвращает (parts, keyboard|None)."""
    import admin_metrics
    import monthly_ops
    import ops_day
    ...
    try:
        ...
        return (["Некорректный месяц."], None)
    ...
    if report_msgs:
        ...
        parts: list[str] = []
        combined = "\n".join(header) + "\n" + body
        parts.extend(split_telegram_html(combined.rstrip()))
        for extra in report_msgs[1:]:
