...
def _parse_event_created_at(row: dict[str, Any]) -> datetime | None:
...
async def insert_feedback_event(row: dict[str, Any]) -> None:
    ...
    created_at = _parse_event_created_at(row)
    try:
        async with _pool.acquire() as conn:
    ...
    try:
        async with _pool.acquire() as conn:
            if pid_int is not None:
                r = await conn.fetchrow("SELECT * FROM problems WHERE id = $1", pid_int)
            else:
                r = await conn.fetchrow(
                    "SELECT * FROM problems WHERE id::text = $1", str(problem_id)
                )
    ...
    try:
        async with _pool.acquire() as conn:
            existing = await conn.fetchrow(
                """
                SELECT id, status, mentions_count, title FROM problems
            ...
            if existing:
                ...
                if (
                    (title or "").strip() == problem_key
                    and prev_title
                    and prev_title != problem_key
                ):
                    title = prev_title
                if prev_status in ("resolved", "ignored"):
...
async def update_problem_title(problem_id: str, title: str) -> bool:
    if _pool is None:
        return False
    try:
        pid = int(problem_id)
    ...
    try:
        async with _pool.acquire() as conn:
            await conn.execute(
                "UPDATE problems SET title = $2, updated_at = now() WHERE id = $1",
                pid,
                title.strip(),
            )
        return True
    except Exception as e:
        print("[postgres update_problem_title]", repr(e))
        return False
...
async def count_unique_users_by_chat(
    ...
    *,
    ...
    """Уникальные telegram_user_id по точкам (все события с user_id)."""
    if _pool is None or not chat_ids:
        return {}
    q = """
        SELECT restaurant_chat_id, COUNT(DISTINCT telegram_user_id) AS n
    ...
    q += " GROUP BY restaurant_chat_id"
    try:
        async with _pool.acquire() as conn:
            rows = await conn.fetch(q, *args)
        return {int(r["restaurant_chat_id"]): int(r["n"]) for r in rows}
    except Exception as e:
        print("[postgres count_unique_users_by_chat]", repr(e))
        return {}
