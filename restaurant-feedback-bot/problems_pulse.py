...
def pr_callback(*parts: str) -> str:
    ...
    return ("pr:" + ":".join(str(p) for p in parts))[:64]
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text="В работу",
                    callback_data=pr_callback("w", cid, problem_id, "ip"),
                )
            ]
        )
    if status != STATUS_RESOLVED:
        rows.append(
            [
                InlineKeyboardButton(
                    text="Решено",
                    callback_data=pr_callback("w", cid, problem_id, "rs"),
                )
            ]
        )
    if status != STATUS_IGNORED:
        rows.append(
            [
                InlineKeyboardButton(
                    text="Игнорировать",
                    callback_data=pr_callback("w", cid, problem_id, "ig"),
                )
            ]
        )
    rows.append(
    ...
    for p in rows[:20]:
        ...
        ik.append(
            [
                InlineKeyboardButton(
                    text=label,
                    callback_data=pr_callback("v", cid, p.id),
                )
            ]
        )
    ik.append(
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    ...
                    callback_data=pr_callback("k", str(chat_id), problem_id),
                )
            ]
        ]
    )
