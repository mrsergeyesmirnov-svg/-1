...
waiting_checklist_note: dict[int, dict] = {}
...
async def checklist_callback_handler(callback: CallbackQuery) -> None:
    ...
    if action in ("chk", "itm") and len(parts) > 5:
        ...
        await callback.answer()
        await _edit_checklist_message(
            callback.message, rec, chat_id, day, kind, item_id=item_id
        )
        return
    ...
    if action == "iss" and len(parts) > 5:
        ...
        items = {i["id"]: i for i in shift_checklists.get_template_items(rec, kind)}
        item_text = items.get(item_id, {}).get("text", "пункт")
        await callback.answer()
        await callback.message.answer(
            f"⚠️ <b>Замечание</b> · {escape(str(item_text))}\n\n"
            "Напишите, что не так (от 3 до 300 символов):",
            parse_mode="HTML",
        )
        return
...
async def manager_menu_handler(message: Message) -> None:
    ...
    waiting_checklist_note.pop(uid, None)
    waiting_chef_survey_comment.pop(uid, None)
...
async def comment_handler(message: Message) -> None:
    ...
    if user_id in waiting_checklist_note:
        flow = waiting_checklist_note.pop(user_id)
        note = text_in.strip()
        if len(note) < 3 or len(note) > 300:
            waiting_checklist_note[user_id] = flow
            await message.answer("Комментарий: от 3 до 300 символов.")
            return
        ...
        rec = chat_record(data, chat_id) or {}
        shift_checklists.set_check_status(
            rec, day, kind, item_id, user_id, status="issue", note=note
        )
        await save_data(data)
        await message.answer(
            "⚠️ Замечание сохранено.",
            parse_mode="HTML",
            reply_markup=shift_checklists.run_checklist_keyboard(
