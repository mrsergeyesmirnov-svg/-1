    ...
    *,
    ...
    if kind == "morning":
        text = (
            ...
            "Откройте бота → «План дня» и заполните план, стоп и расстановку."
        )
    else:
        text = (
            f"⏰ <b>Закрытие дня</b> — {escape(title)}\n\n"
            f"До итогов в чат осталось несколько минут (в <b>{escape(deadline_hm)}</b>).\n"
            "Откройте бота → «Закрытие дня»: выручка и выполнение плана."
        )
    targets = set(problems_pulse.managers_for_chat(data, chat_id))
    for aid in ADMIN_IDS:
        targets.add(aid)
    for mid in targets:
        try:
...
async def _post_morning_to_group(data: dict, chat_id: int, day: str) -> bool:
    rec = chat_record(data, chat_id) or {}
    morning = ops_day.get_morning(rec, day)
    if not ops_day.morning_complete(morning):
        return False
    if morning.get("posted"):
        return False
    title = str(rec.get("title", chat_id))
    text = ops_day.format_morning_group_post(
        title,
        plan=str(morning.get("plan", "")),
        stop_list=str(morning.get("stop_list", "")),
        roster=str(morning.get("roster", "")),
    )
    try:
        await bot.send_message(chat_id, text, parse_mode="HTML")
    except Exception as e:
        print(f"[ops-morning-post] {chat_id}: {e}")
        return False
    ops_day.mark_morning_posted(rec, day)
    return True


async def _post_evening_to_group(data: dict, chat_id: int, day: str) -> bool:
    rec = chat_record(data, chat_id) or {}
    evening = ops_day.get_evening(rec, day)
    if not ops_day.evening_complete(evening):
        return False
    if evening.get("posted"):
        return False
    morning = ops_day.get_morning(rec, day)
    title = str(rec.get("title", chat_id))
    text = ops_day.format_evening_group_post(
        title,
        plan_met=str(evening.get("plan_met", "")),
        revenue=str(evening.get("revenue") or "") or None,
    ...
    try:
        ...
        print(f"[ops-evening-post] {chat_id}: {e}")
        return False
...
async def _send_daily_digest_to_managers(data: dict, chat_id: int, day: str) -> None:
    ...
    tz_name = rec.get("timezone", DEFAULT_TZ)
    baseline = (
        await admin_metrics.count_unique_users_by_chat(
            [chat_id], jsonl_path=FEEDBACK_LOG_PATH, days=30
        )
    ).get(str(chat_id), 0)
    try:
    ...
    raters = (
        await admin_metrics.count_raters_in_period(
            [chat_id], start, end, jsonl_path=FEEDBACK_LOG_PATH
        )
    ).get(str(chat_id), 0)
    eng = admin_metrics.format_engagement_line(raters, baseline, period_label="за сутки")
    ...
    if ops_day.morning_complete(morning):
        ...
        await message.answer(
            ...
            reply_markup=ops_day.morning_actions_keyboard(
                chat_id, posted=bool(morning.get("posted"))
            ),
        )
    else:
        ...
        await message.answer(
            f"<b>План дня</b> · {escape(str(rec.get('title', chat_id)))}\n\n"
            "Шаг 1/3: напишите <b>план на день</b> (списком или текстом).\n"
            f"<i>Автопубликация в чат в {escape(sched['morning_post'])} — или сразу после заполнения.</i>",
            parse_mode="HTML",
        )
...
async def _show_evening_status(message: Message, uid: int, chat_id: int) -> None:
    ...
    if ops_day.evening_complete(evening):
        ...
        await message.answer(
            f"<b>Закрытие дня</b> · {escape(str(rec.get('title', chat_id)))}\n\n"
            "Шаг 1/3: напишите <b>выручку</b> (число или «—» если не делитесь с командой).",
            parse_mode="HTML",
        )
...
async def cmd_help(message: Message) -> None:
    ...
    if is_global_admin(message.from_user.id):
        await message.answer(
            ...
            f"У менеджеров — «Отчёт», «{pulse_model.BTN_SIGNALS}», «{pulse_model.BTN_DAY_PLAN}», «{pulse_model.BTN_DAY_CLOSE}», задания, поддержка.",
            parse_mode="HTML",
        )
    elif await manager_ui_for_user(message.from_user.id):
        await message.answer(
            f"Меню: отчёт, <b>{escape(pulse_model.BTN_SIGNALS)}</b>, "
            f"<b>{escape(pulse_model.BTN_DAY_PLAN)}</b>, <b>{escape(pulse_model.BTN_DAY_CLOSE)}</b>, "
            f"«{escape(pulse_model.BTN_STAFF_OUT)}», «{escape(pulse_model.BTN_TASKS)}».\n"
...
async def cmd_set_ops(message: Message) -> None:
    """Расписание плана дня и закрытия: /set_ops morning 11:30 evening 00:00"""
    if message.chat.type not in ("group", "supergroup"):
    ...
    if not morning and not evening:
        await message.answer(
            ...
            "Напоминания менеджерам: за 5 мин до плана, за 10 мин до итогов.\n"
    ...
    await message.answer(
        f"Операционный день: план в <b>{escape(s['morning_post'])}</b>, "
        f"итоги в <b>{escape(s['evening_post'])}</b> ({escape(str(chats[cid].get('timezone', DEFAULT_TZ)))}).",
        parse_mode="HTML",
    )
            ...
            for cid, info in list(chats.items()):
                ...
                if hm == sched["morning_post"]:
                    ...
                    if not sent_map.get(mkey):
                        try:
                            if await _post_morning_to_group(data, int(cid), morning_day):
                                sent_map[mkey] = True
                                changed = True
                ...
                if hm == sched["evening_post"]:
                    ...
                    if not sent_map.get(ekey):
                        try:
                            posted = await _post_evening_to_group(
                            ...
                            if posted:
                                ...
                                rec = chat_record(data, int(cid)) or {}
                                ev = ops_day.get_evening(rec, evening_day)
                                if not ops_day.evening_complete(ev):
                                    await _ops_nudge_managers(
                                        data,
                                        int(cid),
                                        kind="evening",
...
async def ops_callback_handler(callback: CallbackQuery) -> None:
    ...
    if action in ("mpick", "epick", "spick") and len(parts) > 2:
        try:
            chat_id = int(parts[2])
        except ValueError:
            await callback.answer()
    ...
    if action == "mpub" and len(parts) > 2:
        ...
        ok = await _post_morning_to_group(data, chat_id, day)
        await save_data(data)
        await callback.answer("Опубликовано" if ok else "Сначала заполните план")
        if ok:
    ...
    if action == "epub" and len(parts) > 2:
        ...
        rec = chat_record(data, chat_id) or {}
        day = _ops_evening_day(data, chat_id)
        ok = await _post_evening_to_group(data, chat_id, day)
        await save_data(data)
        await callback.answer("Опубликовано" if ok else "Заполните закрытие дня")
        if ok:
    ...
    if action in ("medit", "eedit") and len(parts) > 2:
        ...
        await callback.answer()
        if action == "medit":
            waiting_ops_morning[uid] = {"chat_id": chat_id, "step": "plan", "data": {}}
            await callback.message.answer("Шаг 1/3: новый <b>план на день</b>.", parse_mode="HTML")
        else:
            waiting_ops_evening[uid] = {"chat_id": chat_id, "step": "revenue", "data": {}}
            await callback.message.answer(
                "Шаг 1/3: напишите <b>выручку</b> (или «—»).", parse_mode="HTML"
            )
        return
    ...
    if action == "pm" and len(parts) > 3:
        chat_id = int(parts[2])
        plan_met = parts[3]
        pending = waiting_ops_evening.get(uid)
        if not pending or pending.get("chat_id") != chat_id:
            await callback.answer("Начните «Закрытие дня» заново", show_alert=True)
            return
        pending["data"]["plan_met"] = plan_met
        pending["step"] = "comment"
        await callback.answer()
        await callback.message.answer(
            "Шаг 3/3: комментарий к итогам (или «—»).", parse_mode="HTML"
        )
        return
    ...
    if action == "tdone" and len(parts) > 2:
        ...
        if ops_day.complete_task(data, tid, uid):
            ...
            await callback.message.answer(
                ops_day.format_tasks_list(tasks),
                parse_mode="HTML",
    ...
    if action == "anew":
        if not ops_day.can_assign_tasks(data, uid, is_global_admin=is_global_admin(uid)):
            ...
            return
        mgrs = ops_day.list_assignable_managers(
            data, uid, is_global_admin=is_global_admin(uid)
        )
        if not mgrs:
            await callback.answer("Нет менеджеров", show_alert=True)
            return
        await callback.answer()
        await callback.message.answer(
            "<b>Кому задание?</b>",
            parse_mode="HTML",
            reply_markup=ops_day.assign_task_managers_keyboard(mgrs),
        )
        return

    if action == "ato" and len(parts) > 2:
        if not ops_day.can_assign_tasks(data, uid, is_global_admin=is_global_admin(uid)):
            await callback.answer("Нет прав", show_alert=True)
            return
        try:
            ...
            await callback.answer()
            return
        waiting_task_assign[uid] = {"to_uid": to_uid}
        await callback.answer()
        await callback.message.answer("Напишите текст задания (до 500 символов):")
        return
...
async def comment_handler(message: Message) -> None:
    ...
    if user_id in waiting_task_assign:
        ...
        if len(body) < 3:
            ...
            return
        task = ops_day.assign_task(
            data,
            from_uid=user_id,
            to_uid=int(assign["to_uid"]),
            chat_id=assign.get("chat_id"),
            text=body,
        )
        await save_data(data)
        await message.answer("✅ Задание назначено.", reply_markup=pulse_model.manager_menu_reply_markup())
        try:
            await bot.send_message(
                int(task["to_uid"]),
                f"📌 <b>Новое задание</b>\n\n{escape(body)}",
    ...
    if user_id in waiting_ops_morning:
        ...
        if step == "plan":
            if len(text_in) < 3:
                await message.answer("План: минимум 3 символа.")
                return
            d["plan"] = text_in
            flow["step"] = "stop"
            await message.answer(
                "Шаг 2/3: <b>стоп-лист</b> (или «—»).", parse_mode="HTML"
            )
            return
        if step == "stop":
            d["stop_list"] = text_in
        ...
        if step == "roster":
            ...
            await message.answer(
                "✅ План сохранён.",
                reply_markup=ops_day.morning_actions_keyboard(chat_id, posted=False),
            )
            preview = ops_day.format_morning_group_post(
                str(rec.get("title", chat_id)),
                plan=d.get("plan", ""),
    ...
    if user_id in waiting_ops_evening:
        ...
        if step == "revenue":
            d["revenue"] = text_in
            flow["step"] = "plan_met_pick"
            await message.answer(
                "Шаг 2/3: выполнен ли план дня?",
                reply_markup=ops_day.evening_plan_met_keyboard(chat_id),
            )
            return
        if step == "comment":
            d["comment"] = text_in
            plan_met = d.get("plan_met")
            if plan_met not in (
                ops_day.PLAN_MET_YES,
                ops_day.PLAN_MET_PARTIAL,
                ops_day.PLAN_MET_NO,
            ):
                waiting_ops_evening[user_id] = flow
                await message.answer("Сначала выберите выполнение плана кнопкой.")
                return
            waiting_ops_evening.pop(user_id, None)
            ...
            await save_data(data)
            await message.answer(
                "✅ Закрытие дня сохранено.",
                reply_markup=ops_day.evening_actions_keyboard(chat_id, posted=False),
            )
            return
