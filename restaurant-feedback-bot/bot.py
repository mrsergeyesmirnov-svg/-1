...
async def _run_monthly_report_for_chat(data: dict, cid: str, info: dict) -> None:
    ...
    for mid in managers:
        ...
        if not parts:
            continue
        first = parts[0]
        if "ответов пока нет" in first or "нет данных" in first:
            continue
        intro = (
    ...
    while True:
        try:
            ...
            for cid, info in list(chats.items()):
                ...
                # Понедельник 10:05 — отчёт за последние 3 недели (не ждём календарный месяц)
                if now_local.weekday() == 0 and hm == "10:05":
                    week_key = now_local.strftime("%G-W%V")
                    month_key = f"{cid}|month3w|{week_key}"
                    if not sent_map.get(month_key):
                        try:
                            await _run_monthly_report_for_chat(data, cid, info)
                            sent_map[month_key] = True
                            changed = True
                            print(f"[month-report] chat={cid}")
                        except Exception as e:
                            print(f"[month-report-fail] {cid}: {e}")
