...
def build_report_html(
    *,
    ...
    lines = [
        ...
        f"Оценок смен: <b>{n_ratings}</b> · комментариев: <b>{n_comments}</b>",
        "",
    ]
    if engagement_line:
    ...
    if problems:
        lines.append("<b>Что мешало на смене</b>")
    ...
    if comments:
        ...
        for text, plab_c, ts in comments:
            tag = f"<i>{escape(plab_c)}</i> — " if plab_c else ""
            when = ""
        ...
        lines.append("<i>Текстовых комментариев за смену нет.</i>\n")
    ...
    *,
    ...
    if not current:
        return [
            ...
            "Попросите команду ответить из рабочей группы (кнопка «Рассказать в личке»)."
        ]
    ...
    if not multi:
        ...
        report_msgs = build_report_html(
            ...
            current=[e for e in current if e.restaurant_chat_id == cid_i or e.restaurant_chat_id is None],
            previous=[e for e in previous if e.restaurant_chat_id == cid_i or e.restaurant_chat_id is None],
            problem_labels=problem_labels,
            engagement_line=admin_metrics.format_engagement_line(
                raters_map.get(cid, 0),
                baseline_map.get(cid, 0),
                period_label=plabel,
            ),
            problem_stats_line=ops_day.format_problem_stats_block(pstats),
            ops_summary_line=ops_line,
            departures_line=(
