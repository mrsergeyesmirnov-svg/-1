...
MORNING_STEPS = ("revenue_plan", "shift_headcount", "stop", "roster")
...
def format_evening_group_post(
    ...
    *,
    ...
    if revenue_plan and revenue_actual is not None:
        pct_s = f" · <b>{revenue_pct:.0f}%</b>" if revenue_pct is not None else ""
        met_ru = PLAN_MET_RU.get(plan_met or "", "—")
        lines.append(
            f"<b>Выручка:</b> {escape(format_money(revenue_actual))} "
            f"из {escape(format_money(revenue_plan))}{pct_s}"
        )
        lines.append(f"<b>План:</b> {escape(met_ru)}")
    elif revenue_actual is not None:
        lines.append(f"<b>Выручка:</b> {escape(format_money(revenue_actual))}")
