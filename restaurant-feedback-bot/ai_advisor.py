...
Ты НЕ морализатор и НЕ родитель управу. Не читай нотаций, не запрещай\
 «орать/кричать», не стыди. Управ — взрослый. Дай смысл и формат разговора.
...
Верни ТОЛЬКО JSON без markdown:
{
  ...
  "tone": "цель и формат тет-а-тет: зачем разговор, какую позу занять (слушать, понять систему), как задавать вопросы чтобы человек сам покопался",
...
Правила тет-а-тет:
- Смысл: понять систему и вырасти вместе, не «вызвать и надавить».
- Вопросы — чтобы человек сам покопался; не допрос и не поиск виноватого.
- В tone опиши цель и формат (слушать → вопрос → ясность), без запретов и нравоучений.

actions — конкретные шаги. quotes — только из данных, не выдумывай.\
...
def render_advice_report(
    ...
    *,
    ...
    if risk:
        ...
        lines.append("")
    if tone:
        lines.append("🧭 <b>Смысл разговора</b>")
    ...
    if actions:
        ...
        lines.append("")
    if questions:
        lines.append("🗣 <b>Вопросы тет-а-тет</b>")
...
def template_report_data(
    ...
    return {
        ...
        "tone": (
            "Цель тет-а-тет — понять, что ломает систему, и вместе найти ясность. "
            "Сначала слушание, потом вопросы, чтобы человек сам покопался. "
            "Не допрос и не поиск виноватого: после разговора у обоих должна быть ясность, что делать дальше."
        ),
        "actions": [
        ...
        "cta": "Сегодня один тет-а-тет: услышать и зафиксировать одну ясность на завтра.",
...
async def build_advice(
    ...
    *,
    ...
    context += (
        ...
        "В tone — смысл и формат разговора, без нравоучений и запретов управу."
    )
    try:
        resp = await client.chat.completions.create(
            model=OPENAI_MODEL,
