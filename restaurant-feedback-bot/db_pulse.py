...
async def fetch_problems_for_chat(
    ...
    *,
    ...
    q += (
        ...
        "WHEN 'resolved' THEN 2 ELSE 3 END, mentions_count DESC"
    )
    try:
        async with _pool.acquire() as conn:
    ...
    *,
    ...
    try:
        async with _pool.acquire() as conn:
            ...
            if existing:
                new_status = (
                    "new" if existing["status"] == "resolved" else existing["status"]
                )
                row = await conn.fetchrow(
                    ...
                    UPDATE problems SET
                        ...
                        status = $5,
                        resolved_at = CASE WHEN $5 = 'resolved' THEN resolved_at ELSE NULL END
                    WHERE id = $1
                    RETURNING *
                    """,
                    existing["id"],
