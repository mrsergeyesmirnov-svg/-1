...
async def _pr_chat_id_from_parts(
    data: dict, uid: int, parts: list[str], action: str
) -> int | None:
    """Точка из callback_data (новые кнопки) или из сессии / единственной точки."""
    cid_raw: str | None = None
    if action in ("l", "arch", "add", "sync") and len(parts) > 2:
        cid_raw = parts[2]
    elif action == "v" and len(parts) > 3:
        cid_raw = parts[2]
    elif action == "w" and len(parts) > 4:
        cid_raw = parts[2]
...
async def problems_callback_handler(callback: CallbackQuery) -> None:
    ...
    chat_id = await _pr_chat_id_from_parts(data, uid, parts, action)
    if chat_id is None:
        await callback.answer(
