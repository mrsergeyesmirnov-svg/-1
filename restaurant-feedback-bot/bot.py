...
async def _run_weekly_problems_for_chat(
    ...
    digest = problems_pulse.format_weekly_digest(rows)
    await _post_problem_to_group(chat_id, digest)
    mgr_text = problems_pulse.format_manager_problems_report(
        str(title), rows, sync_notes=notes or None
    )
    await _notify_managers_problem_report(data, chat_id, mgr_text)
...
async def _show_problems_for_manager(
    ...
    text = problems_pulse.format_problem_list(rows, title="Управление проблемами")
    await message.answer(
        text,
        parse_mode="HTML",
        reply_markup=problems_pulse.problems_list_keyboard(rows),
    )


async def _show_buttons_config(
    message: Message,
    uid: int,
    chat_id: int,
) -> None:
    ...
    title = rec.get("title", str(chat_id)) if rec else str(chat_id)
    await message.answer(
        survey_buttons.format_config_message(data, chat_id, str(title)),
        parse_mode="HTML",
        reply_markup=survey_buttons.config_keyboard(data, chat_id),
    )
...
async def cmd_help(message: Message) -> None:
    ...
    if is_global_admin(message.from_user.id):
        await message.answer(
            ...
            "У менеджеров — «Отчёт», «Проблемы» (<code>/problems</code>), подписка, поддержка.",
            parse_mode="HTML",
        )
    elif await manager_ui_for_user(message.from_user.id):
        await message.answer(
            "У вас есть меню: отчёт, <b>проблемы</b>, подписка, поддержка.\n"
...
async def problem_handler(callback: CallbackQuery) -> None:
    ...
    allowed = (
        ...
        else {b["code"] for b in survey_buttons.DEFAULT_BUTTONS}
    )
...
async def cmd_problems(message: Message) -> None:
    if message.chat.type != "private":
        await message.answer("Команда <code>/problems</code> — в личке с ботом.", parse_mode="HTML")
        return
    uid = message.from_user.id
    data = await load_data()
...
async def problems_callback_handler(callback: CallbackQuery) -> None:
    ...
    if chat_id is None:
        await callback.answer("Сначала выберите точку: /problems", show_alert=True)
        return
    ...
    if action == "sync":
        ...
        mgr_text = problems_pulse.format_manager_problems_report(
            str(title), rows, sync_notes=notes or ["Без изменений по порогам"]
        )
        await _notify_managers_problem_report(
            data, chat_id, mgr_text, exclude_uid=uid
        )
...
async def survey_buttons_callback(callback: CallbackQuery) -> None:
    ...
    chat_id = await _resolve_manager_problem_chat(data, uid)
    if chat_id is None:
        await callback.answer("Сначала откройте «Проблемы» и выберите точку.", show_alert=True)
        return

    if action == "cfg":
        await callback.answer()
        await _show_buttons_config(callback.message, uid, chat_id)
        return
    ...
    if action == "ed" and len(parts) > 2:
        ...
        await callback.answer()
        await callback.message.answer(
            f"Новый текст для кнопки <code>{escape(code)}</code> (2–40 символов):",
            parse_mode="HTML",
        )
        return
    ...
    if action == "add":
        ...
        await callback.answer()
        await callback.message.answer(
            "Текст новой кнопки (2–40 символов), например: <i>Лицензии iiko</i>",
            parse_mode="HTML",
        )
        return
...
async def _apply_problem_status_change(
    ...
    if not prob:
        await message.answer("Не удалось обновить проблему.")
        return
    await save_data(data)
    st_ru = problems_pulse.STATUS_RU.get(status, status)
    await message.answer(
    ...
    mgr_note = (
        f"📋 <b>Проблема обновлена</b> — {escape(str(rec.get('title', prob.restaurant_chat_id)))}\n\n"
...
async def manager_menu_handler(message: Message) -> None:
    ...
    elif t == pulse_model.BTN_PROBLEMS:
...
async def comment_handler(message: Message) -> None:
    ...
    if user_id in waiting_button_edit:
        ...
        await save_data(data)
        await message.answer(
            "Сохранено. Так кнопку увидят сотрудники при следующем опросе.",
            reply_markup=pulse_model.manager_menu_reply_markup(),
        )
        await _show_buttons_config(message, user_id, chat_id)
        return
