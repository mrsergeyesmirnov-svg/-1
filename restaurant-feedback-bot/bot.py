...
async def _show_morning_status(message: Message, uid: int, chat_id: int) -> None:
    ...
    if ops_day.morning_complete(morning):
        ...
        await message.answer(
            f"<b>План дня</b> · {escape(str(rec.get('title', chat_id)))}\n\n"
            "Шаг 1/4: <b>план по выручке</b> на сегодня (число, например 850000).\n"
            f"<i>Автопубликация в чат в {escape(sched['morning_post'])}.</i>",
            parse_mode="HTML",
        )
...
async def cmd_help(message: Message) -> None:
    ...
    if is_global_admin(message.from_user.id):
        await message.answer(
            ...
            f"У менеджеров — меню из папок: «{pulse_model.BTN_FOLDER_ANALYTICS}», «{pulse_model.BTN_FOLDER_SHIFT}», «{pulse_model.BTN_FOLDER_MORE}».",
            parse_mode="HTML",
        )
...
async def ops_callback_handler(callback: CallbackQuery) -> None:
    ...
    if action in ("medit", "eedit") and len(parts) > 2:
        ...
        if action == "medit":
            ...
            await callback.message.answer(
                "Шаг 1/4: <b>план по выручке</b> (число).", parse_mode="HTML"
            )
        else:
...
async def manager_menu_handler(message: Message) -> None:
    ...
    if t == pulse_model.BTN_FOLDER_SHIFT:
        await message.answer(
            "<b>Смена</b>\nПлан утром, закрытие вечером, кадры, задания.",
            parse_mode="HTML",
...
async def comment_handler(message: Message) -> None:
    ...
    if user_id in waiting_ops_morning:
        ...
        if step == "revenue_plan":
            ...
            await message.answer(
                "Шаг 2/4: <b>сколько человек на смене</b> сегодня? (число)",
                parse_mode="HTML",
            )
            return
        if step == "shift_headcount":
            ...
            flow["step"] = "stop"
            await message.answer(
                "Шаг 3/4: <b>стоп-лист</b> (или «—»).", parse_mode="HTML"
            )
            return
        if step == "stop":
            d["stop_list"] = text_in
            flow["step"] = "roster"
            await message.answer(
                "Шаг 4/4: <b>расстановка</b> (или «—»).", parse_mode="HTML"
            )
            return
        if step == "roster":
            d["roster"] = text_in
    ...
    if user_id in waiting_ops_evening:
        ...
        if step == "comment":
            ...
            await message.answer(
                f"✅ Закрытие сохранено: {escape(met)}{pct_s}",
