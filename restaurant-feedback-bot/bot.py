...
async def _post_morning_to_group(
    ...
    rec = chat_record(data, chat_id) or {}
    if not shift_checklists.shift_gate_complete(rec, day, "opening"):
        return "incomplete", "opening_checklist"
...
async def _post_evening_to_group(
    ...
    """(ok|incomplete|already_posted|send_failed, detail)"""
    rec = chat_record(data, chat_id) or {}
    if not shift_checklists.shift_gate_complete(rec, day, "closing"):
...
async def _show_morning_status(
    ...
    if (
        ...
        and shift_checklists.get_template_items(rec, "opening")
        and not shift_checklists.shift_gate_complete(rec, day, "opening")
    ):
        await message.answer(
            shift_checklists.format_checklist_prompt(rec, title, day, "opening"),
            parse_mode="HTML",
            reply_markup=shift_checklists.run_checklist_keyboard(
                rec, chat_id, day, "opening"
            ),
        )
        return
    morning = ops_day.get_morning(rec, day)
...
async def _show_evening_status(
    message: Message, uid: int, chat_id: int, *, skip_closing_gate: bool = False
) -> None:
    data = await load_data()
    rec = chat_record(data, chat_id) or {}
    tz_name = rec.get("timezone", DEFAULT_TZ)
...
async def checklist_callback_handler(callback: CallbackQuery) -> None:
    ...
    if action == "cfg" and len(parts) > 3:
        kind = "opening" if parts[2] == "o" else "closing"
        chat_id = int(parts[3])
        if not await _manager_can_access_chat(data, uid, chat_id):
            await callback.answer("Нет доступа.", show_alert=True)
            return
    ...
    if action == "chk" and len(parts) > 5:
        ...
        await callback.message.edit_text(
            shift_checklists.format_checklist_prompt(rec, title, day, kind),
            parse_mode="HTML",
            reply_markup=shift_checklists.run_checklist_keyboard(
...
async def ops_callback_handler(callback: CallbackQuery) -> None:
    ...
    if action == "mpub" and len(parts) > 2:
        ...
        toast = {
            "ok": "Опубликовано",
            "incomplete": "Сначала чек-лист открытия и план",
        ...
        if result == "ok":
            ...
            if detail == "opening_checklist":
                await callback.message.answer(
                    "Сначала отметьте все пункты <b>открытия смены</b> "
                    f"({pulse_model.BTN_CHECKLISTS} → План дня).",
                    parse_mode="HTML",
                )
            else:
                await callback.message.answer(
...
async def manager_menu_handler(message: Message) -> None:
    ...
    if t == pulse_model.BTN_FOLDER_SHIFT:
        await message.answer(
            "<b>Смена</b>\n"
            "План, закрытие, стоп-лист, оценка кухни, сообщение команде.",
            parse_mode="HTML",
    ...
    if t == pulse_model.BTN_REPORT:
        ...
        if chat_id is not None:
            ...
            await message.answer(
                ...
                "Настройте пункты открытия и закрытия.",
                parse_mode="HTML",
                reply_markup=shift_checklists.checklist_menu_keyboard(chat_id),
            )
...
async def report_pdf_export_handler(callback: CallbackQuery) -> None:
    ...
    try:
        if month_key:
            parts, _kb = await report_pulse.build_calendar_month_reports(
                data,
                uid,
                month_key,
                is_global_admin=is_global_admin(uid),
    ...
    try:
        pdf_bytes = pdf_report.build_pdf_bytes(
            parts, title=f"Pulse Team — {title}"
        )
        fname = pdf_report.pdf_filename(title, plabel)
    except Exception as e:
        print(f"[report-pdf-build] uid={uid}", repr(e))
        await callback.message.answer(
            "PDF временно недоступен. Убедитесь, что на сервере установлен fpdf2."
        )
        return
