...
CHEF_MENU_BUTTONS = frozenset(
    {BTN_RATE_SHIFT, BTN_STOP_LIST, BTN_STOP_CURRENT, BTN_SUPPORT}
)
...
def has_manager_access(data: dict[str, Any], user_id: int) -> bool:
    ...
    return any(
        isinstance(p, dict) and p.get("role") in (ROLE_NETWORK_ADMIN, ROLE_LOCATION_ADMIN)
        for p in profiles
    )
    ...
    for p in profiles:
        ...
        if role == ROLE_NETWORK_ADMIN and role in roles and oid:
            for cid, _ in list_chats_for_org(data, str(oid)):
                ids.add(cid)
        elif role in (ROLE_LOCATION_ADMIN, ROLE_CHEF) and role in roles:
...
def allowed_chat_ids_for_manager(data: dict[str, Any], user_id: int) -> set[str]:
    ...
    return _location_ids_from_profiles(
        ...
        roles=frozenset({ROLE_NETWORK_ADMIN, ROLE_LOCATION_ADMIN}),
    )
...
def set_manager_binding(
    ...
    if role == ROLE_NETWORK_ADMIN:
        locs: list[str] = []
    elif role in (ROLE_LOCATION_ADMIN, ROLE_CHEF):
...
def text_subscription_status(data: dict[str, Any], user_id: int) -> str:
    ...
    for p in profiles:
        ...
        role = p.get("role")
        if role == ROLE_NETWORK_ADMIN:
            role_ru = "управляющий сетью"
        elif role == ROLE_CHEF:
            role_ru = "шеф"
        else:
            role_ru = "менеджер точки"
        lines.append(
...
def manager_menu_shift_markup():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text=BTN_STOP_LIST), KeyboardButton(text=BTN_MONTH_PLAN)],
...
def chef_menu_reply_markup():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text=BTN_STOP_LIST), KeyboardButton(text=BTN_STOP_CURRENT)],
...
def admin_commands_reference_chunks() -> list[str]:
    ...
    return [
        (
            ...
            "<b>Привязка людей</b>\n"
            "<code>/link_manager ID org_id network</code> — управляющий сети\n"
            "<code>/link_manager ID org_id location CHAT_ID</code> — менеджер точки\n"
            "<code>/link_manager ID org_id chef CHAT_ID</code> — шеф (стоп-лист)\n\n"
            "<b>В группе точки</b>\n"
        ...
        (
            ...
            f"<b>{BTN_FOLDER_SHIFT}</b> — {BTN_DAY_PLAN}, {BTN_DAY_CLOSE}, "
            f"{BTN_STOP_LIST}, {BTN_MONTH_PLAN}, {BTN_STAFF_OUT}, {BTN_TASKS}\n"
            f"<b>{BTN_FOLDER_MORE}</b> — {BTN_SUBSCRIPTION}, {BTN_SUPPORT}, {BTN_CONNECT}\n\n"
            ...
            f"{BTN_STOP_LIST} — стоп-лист в группу сразу\n"
            f"{BTN_STOP_CURRENT} — посмотреть актуальный стоп\n"
            f"{BTN_SUPPORT}\n"
