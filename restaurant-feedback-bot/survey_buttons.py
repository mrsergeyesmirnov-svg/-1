...
def format_config_message(data: dict[str, Any], chat_id: int, chat_title: str) -> str:
    ...
    n = count_enabled(buttons)
    lines = [
        "⚙️ <b>Быстрые кнопки опроса</b>",
        f"Точка: <b>{escape(chat_title)}</b>",
        "",
        f"Активных: <b>{n}</b> из <b>{MAX_ENABLED}</b> (максимум в опросе).",
        "Стандартные темы можно <b>выключить</b>. Свои кнопки — "
        "<b>изменить текст</b> или <b>удалить</b>.",
        "",
    ]
    for b in buttons:
        mark = "✅" if b.get("enabled") else "⬜"
        tag = "" if b.get("builtin") else " · своя"
        lines.append(f"{mark} {escape(str(b['label']))}{tag}")
    if n >= MAX_ENABLED:
        lines.append("")
        lines.append(
            "<i>Чтобы включить другую — сначала выключите одну из активных.</i>"
        )
    return "\n".join(lines)
...
def config_keyboard(data: dict[str, Any], chat_id: int):
    ...
    if count_enabled(buttons) < MAX_ENABLED and custom_count < MAX_CUSTOM:
        rows.append(
            [InlineKeyboardButton(text="➕ Добавить кнопку", callback_data="pb:add")]
        )
    rows.append([InlineKeyboardButton(text="← К проблемам", callback_data="pr:l")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
