    ...
    *,
    ...
    for d in store:
        if (
            ...
            if prev_status in (STATUS_RESOLVED, STATUS_IGNORED) and count >= threshold:
                d["status"] = STATUS_NEW
                d["resolved_at"] = None
...
async def sync_problems_from_period(
    ...
    *,
    ...
    existing_keys = {p.problem_key for p in existing_rows}
...
def format_archive_list(
    ...
    *,
    ...
    lines = [
        f"<b>{header}</b>",
        "<i>Можно открыть тему и сменить статус — вернётся в активный список.</i>",
        "",
    ]
    for p in rows[:25]:
        em = STATUS_EMOJI.get(p.status, "⚪")
...
def format_weekly_digest(rows: list[ProblemRow]) -> str:
    ...
    if not (resolved or in_prog or new):
        ...
        lines.append(
            "<i>Анонимные отклики → действия команды. Ответить можно по кнопке из чата.</i>"
        )
    return "\n".join(lines)
...
def problems_list_keyboard(rows: list[ProblemRow], *, archive_count: int = 0):
    from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
    ...
    ik.append(
        [InlineKeyboardButton(text="➕ Своя тема", callback_data="pr:add")]
    )
    ik.append(
        [
            InlineKeyboardButton(
    ...
    ik.append([InlineKeyboardButton(text=arch_label, callback_data="pr:arch")])
    ik.append(
        [
            InlineKeyboardButton(
...
def problems_archive_keyboard(rows: list[ProblemRow]):
    from aiogram.types import InlineKeyboardButton, InlineKeyboardMarkup
    ...
    for p in rows[:25]:
        ...
        ik.append(
            [InlineKeyboardButton(text=f"{em} {short}", callback_data=f"pr:v:{p.id}")]
        )
    ik.append([InlineKeyboardButton(text="← К активным", callback_data="pr:l")])
    return InlineKeyboardMarkup(inline_keyboard=ik)
