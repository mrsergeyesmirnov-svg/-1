...
def all_thresholds_map(data: dict[str, Any], chat_id: int) -> dict[str, int]:
    """Пороги всех кнопок точки (включая выключенные) — для синхронизации сигналов."""
    return {b["code"]: int(b.get("threshold", 3)) for b in get_buttons(data, chat_id)}
...
def format_config_message(data: dict[str, Any], chat_id: int, chat_title: str) -> str:
    ...
    return (
        ...
        "<i>Свои кнопки — темы из опроса; при наборе порога отметок попадают в «Горящие вопросы».</i>"
    )
...
def config_keyboard(data: dict[str, Any], chat_id: int):
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=f"{mark} {short}",
                    callback_data=f"pb:t:{cid}:{code}"[:64],
                )
            ]
        )
        if not b.get("builtin"):
            rows.append(
                [
                    InlineKeyboardButton(
                        text="✏️",
                        callback_data=f"pb:ed:{cid}:{code}"[:64],
                    ),
                    InlineKeyboardButton(
                        text="🗑",
                        callback_data=f"pb:del:{cid}:{code}"[:64],
                    ),
                ]
            )
    ...
    if count_enabled(buttons) < MAX_ENABLED and custom_count < MAX_CUSTOM:
        rows.append(
            [InlineKeyboardButton(text="➕ Своя кнопка", callback_data=f"pb:add:{cid}")]
        )
