...
def is_stale_untouched_problem(p: ProblemRow, *, days: int = STALE_PROBLEM_DAYS) -> bool:
    """Новая тема без перевода «в работе» дольше N дней."""
    if p.status != STATUS_NEW:
        return False
    ref = p.first_detected_at or p.last_detected_at
    if not ref:
        return False
    if ref.tzinfo is None:
        ref = ref.replace(tzinfo=ZoneInfo("UTC"))
    age = datetime.now(ref.tzinfo) - ref
    return age >= timedelta(days=days)


async def stale_problems_for_chat(
    data: dict[str, Any], chat_id: int, *, days: int = STALE_PROBLEM_DAYS
) -> list[ProblemRow]:
    rows = await list_problems_for_chat(data, chat_id, view=VIEW_ACTIVE)
    return [p for p in rows if is_stale_untouched_problem(p, days=days)]


async def format_network_stale_problems_digest(
    data: dict[str, Any], org_id: str, *, days: int = STALE_PROBLEM_DAYS
) -> str | None:
    """Сводка для управляющего сети: точки, где проблемы «висят» без статуса."""
    from pulse_model import list_chats_for_org

    lines = [
        "⚠️ <b>Проблемы без реакции</b>",
        f"<i>темы в статусе «новая» более {days} дн. — нужен разбор менеджеров точек</i>",
        "",
    ]
    any_row = False
    for cid, title in list_chats_for_org(data, str(org_id)):
        stale = await stale_problems_for_chat(data, int(cid), days=days)
        if not stale:
            continue
        any_row = True
        lines.append(f"<b>{escape(title)}</b> — {len(stale)} шт.")
        for p in stale[:3]:
            lines.append(f"  • {escape(p.title)} ({p.mentions_count})")
        if len(stale) > 3:
            lines.append(f"  <i>…ещё {len(stale) - 3}</i>")
        lines.append("")
    if not any_row:
        return None
    lines.append(
        f"<i>Напомните менеджерам: «{pulse_model.BTN_SIGNALS}» → статус «в работе» или «решена».</i>"
    )
    return "\n".join(lines).rstrip()
...
def format_manager_problems_report(
    ...
    *,
    ...
    lines = [
        ...
        "<i>Напоминание: проставьте статусы — «в работе» или «решена».</i>",
        "",
        _signals_list_body(rows, sync_days=SIGNALS_SYNC_DAYS),
    ]
    stale = [p for p in rows if is_stale_untouched_problem(p)]
    if stale:
        lines.insert(
            3,
            f"⚠️ <b>{len(stale)}</b> тем без движения более "
            f"{STALE_PROBLEM_DAYS} дн. — возьмите в работу.",
        )
        lines.insert(4, "")
    if sync_notes:
        lines.append("")
...
def chefs_for_chat(data: dict[str, Any], chat_id: int) -> list[int]:
    """Telegram user_id шефов точки."""
    import pulse_model
    ...
    for uid_s, profiles in data.get("managers", {}).items():
        ...
        for p in profiles:
            ...
            if p.get("organization_id") != org_id and org_id:
                continue
            if p.get("role") != pulse_model.ROLE_CHEF:
                continue
            locs = [str(x) for x in (p.get("location_chat_ids") or [])]
            if cid in locs:
                out.add(uid)
...
def ops_staff_for_chat(data: dict[str, Any], chat_id: int) -> list[int]:
    """Менеджеры и шефы точки — для операционных напоминаний."""
    return list(set(managers_for_chat(data, chat_id)) | set(chefs_for_chat(data, chat_id)))
