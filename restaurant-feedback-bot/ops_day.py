...
EVENING_STEPS = ("revenue", "comment", "shift_summary")

SHIFT_SUMMARY_LABEL = "Краткое описание смены: сложности и сильные стороны"
SHIFT_SUMMARY_MIN_LEN = 10
SHIFT_SUMMARY_MAX_LEN = 500

PLAN_PARTIAL_PCT = 85  # >=85% — частично, >=100% — выполнен


def shift_summary_valid(text: str) -> tuple[bool, str | None]:
    t = (text or "").strip()
    if len(t) < SHIFT_SUMMARY_MIN_LEN:
        return False, f"Описание: от {SHIFT_SUMMARY_MIN_LEN} до {SHIFT_SUMMARY_MAX_LEN} символов."
    if len(t) > SHIFT_SUMMARY_MAX_LEN:
        return False, f"Не больше {SHIFT_SUMMARY_MAX_LEN} символов."
    if t in ("—", "-"):
        return False, "Это поле обязательно — опишите сложности и сильные стороны смены."
    return True, None


def shift_summary_text(evening: dict[str, Any]) -> str:
    return str(evening.get("shift_summary") or "").strip()
...
def evening_complete(evening: dict[str, Any]) -> bool:
    if parse_amount(str(evening.get("revenue") or "")) is None:
        return False
    ok, _ = shift_summary_valid(shift_summary_text(evening))
    return ok
...
def _normalize_stop_item_key(text: str) -> str:
    t = re.sub(r"\s+", " ", text.strip().lower())
    t = re.sub(r"^[•\-—*]\s*", "", t)
    return t


def _clean_stop_line(raw: str) -> str:
    line = raw.strip()
    if not line:
        return ""
...
def parse_stop_list_items(text: str) -> list[str]:
    """Позиции из текста стоп-листа за один день."""
    ...
    for part in re.split(r"[\n\r]+", text):
        ...
        if not part:
            continue
        if part.count(",") >= 2 or ("," in part and len(part) < 100):
            chunks = re.split(r"[,;]", part)
        else:
            chunks = [part]
        for piece in chunks:
            cleaned = _clean_stop_line(piece)
            if cleaned:
                out.append(cleaned)
    return out


def aggregate_stop_list_frequency(
    rec: dict[str, Any],
    start: datetime,
    end: datetime,
    tz_name: str,
    *,
    ...
    """Частота позиций в стоп-листе: сколько дней за период позиция была в стопе."""
    ...
    while cur <= end_d:
        ...
        text = chef_stop_text_for_day(rec, day_key)
        seen_today: set[str] = set()
        for item in parse_stop_list_items(text):
            key = _normalize_stop_item_key(item)
            if len(key) < 2 or key in _STOP_SKIP_KEYS:
                continue
            ...
            if not prev or len(item.strip()) > len(prev):
                display[key] = item.strip()
        cur += timedelta(days=1)

    return [
        (display[k], counter[k])
        for k, _ in counter.most_common(top_n)
        if counter[k] > 0
    ]


def format_stop_list_stats_block(
    ...
    *,
    ...
    if not freq:
        if show_empty:
            return (
                "<b>Чаще всего в стоп-листе</b>\n"
                "<i>За период нет записей стоп-листа.</i>"
            )
        return None
    lines = [
        "<b>Чаще всего в стоп-листе</b>",
        "<i>сколько дней позиция была в стопе</i>",
    ]
    for name, n in freq:
        lines.append(f"• {escape(name)} — <b>{n}</b>")
    return "\n".join(lines)


def stop_list_line_for_period(
    rec: dict[str, Any],
    start: datetime,
    end: datetime,
    tz_name: str,
    *,
    ...
    if not isinstance(rec, dict):
        return None
    freq = aggregate_stop_list_frequency(rec, start, end, tz_name)
    return format_stop_list_stats_block(freq, show_empty=show_empty)
    ...
    ev.update(
        {
            ...
            "shift_summary": shift_summary.strip()[:SHIFT_SUMMARY_MAX_LEN],
            "by_uid": by_uid,
            "updated_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        }
    )
...
def day_was_closed(rec: dict[str, Any], day: str) -> bool:
    """Смена закрыта — только когда итог дня опубликован или сохранён черновик закрытия."""
    evening = get_evening(rec, day)
    if evening.get("posted"):
        return True
    return evening_complete(evening)
...
async def build_daily_manager_digest(
    ...
    *,
    ...
    if evening_complete(evening):
        ...
        summary = shift_summary_text(evening)
        if summary:
            short = summary if len(summary) <= 220 else summary[:217] + "…"
            lines.append(
                f"<b>{escape(SHIFT_SUMMARY_LABEL)}:</b>\n<i>{escape(short)}</i>"
            )
    else:
...
def collect_shift_summaries(
    rec: dict[str, Any],
    start: datetime,
    end: datetime,
    tz_name: str,
) -> list[dict[str, Any]]:
    """Описания смен менеджера за период (только для отчётов, не для группы)."""
    ...
    for day_key, bucket in days_map.items():
        ...
        if day_start < start or day_start > end:
            continue
        out.append({"day": day_key, "text": summary})
    out.sort(key=lambda x: x["day"], reverse=True)
    return out


def format_shift_summaries_block(
    ...
    lines = [
        f"<b>{escape(SHIFT_SUMMARY_LABEL)}</b>",
        "<i>только для руководства — в группу не уходит</i>",
    ]
    for row in rows[:limit]:
        ...
        try:
            ...
            day_h = d.strftime("%d.%m")
        except ValueError:
            day_h = str(row.get("day", ""))
        lines.append(f"• <b>{escape(day_h)}</b> — «{escape(text)}»")
    if len(rows) > limit:
        lines.append(f"<i>…ещё {len(rows) - limit} за период</i>")
    return "\n".join(lines)
