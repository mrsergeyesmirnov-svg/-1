...
def text_subscription_status(data: dict[str, Any], user_id: int) -> str:
    ...
    for p in profiles:
        ...
        lines.append(f"• **{name}** (`{oid}`)\n  подписка: **{sub_ru}** · ваша роль: {role_ru}\n")
    return "\n".join(lines)
...
def text_report_stub() -> str:
    return (
        "**Отчёт**\n\n"
        "Здесь появится недельный и по запросу **ИИ-разбор**: динамика оценок, темы из комментариев, "
        "**алерты** и **рекомендации** по шагам улучшения (не только средняя оценка).\n\n"
        "Сейчас все ответы уже пишутся в служебный лог событий — их можно скармливать модели по расписанию "
        "или кнопке «Обновить отчёт».\n\n"
        "_MVP: выгрузка в таблицы / ручной разбор — следующий шаг разработки._"
    )
...
def text_support(support_username: str | None) -> str:
    ...
    if su:
        return (
            "**Поддержка**\n\n"
            f"Напишите нам в Telegram: [@{su}](https://t.me/{su})\n\n"
            "Кратко укажите сеть, точку и суть вопроса — ответим как можно быстрее."
        )
    return (
        "**Поддержка**\n\n"
        "Контакт поддержки задаётся администратором бота (переменная `SUPPORT_USERNAME` в `.env`)."
    )
...
def text_connect_point(bot_username: str) -> str:
    ...
    return (
        "**Как подключить точку**\n\n"
        "1. Создайте групповой чат официантов/смены (лучше супергруппа).\n"
        "2. Добавьте в чат бота и дайте право **удалять сообщения** не обязательно, но удобно для сервиса.\n"
        "3. Администратор Pulse привяжет чат к вашей **организации** командой `/link_org …` в этом чате "
        "(или заранее через поддержку).\n"
        "4. В чате задайте время напоминания: `/settime 22:00` и при необходимости `/timezone Europe/Moscow`.\n\n"
        f"Быстрое добавление в группу: [добавить @{un} в группу](https://t.me/{un}?startgroup=open).\n\n"
        "Оценки сотрудников идут **только в личку** по кнопке из группы — так ответ привязан к точке."
    )
