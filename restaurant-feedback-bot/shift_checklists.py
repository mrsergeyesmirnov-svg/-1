...
def _remark_key(kind: CheckKind) -> str:
    if kind in ("opening", "opening_waiters"):
        return "opening_waiters_checklist_remark"
    if kind == "opening_hosts":
        return "opening_hosts_checklist_remark"
    if kind == "opening_manager":
        return "opening_manager_checklist_remark"
    return "closing_checklist_remark"


def get_remark(rec: dict[str, Any], day: str, kind: CheckKind) -> dict[str, Any] | None:
    bucket = ops_day.get_day_bucket(rec, day)
    raw = bucket.get(_remark_key(kind))
    return raw if isinstance(raw, dict) else None


def remark_step_done(rec: dict[str, Any], day: str, kind: CheckKind) -> bool:
    if not all_checked(rec, day, kind):
        return False
    remark = get_remark(rec, day, kind)
    if remark:
        if remark.get("skipped"):
            return True
        return len(str(remark.get("text") or "").strip()) >= 3
    bucket = ops_day.get_day_bucket(rec, day)
...
def set_remark(
    rec: dict[str, Any], day: str, kind: CheckKind, uid: int, text: str
) -> tuple[bool, str | None]:
    text = text.strip()
    if len(text) < 3 or len(text) > 500:
        return False, "Замечание: от 3 до 500 символов."
    bucket = ops_day.get_day_bucket(rec, day)
    bucket[_remark_key(kind)] = {
        "text": text[:500],
        "skipped": False,
        "by_uid": uid,
        "at": datetime.now().astimezone().isoformat(timespec="seconds"),
    }
    _sync_opened_key(rec, day, kind)
    return True, None


def set_remark_skipped(
    rec: dict[str, Any], day: str, kind: CheckKind, uid: int
) -> None:
    bucket = ops_day.get_day_bucket(rec, day)
    bucket[_remark_key(kind)] = {
        "text": "",
...
def clear_remark(rec: dict[str, Any], day: str, kind: CheckKind) -> None:
    bucket = ops_day.get_day_bucket(rec, day)
    bucket.pop(_remark_key(kind), None)
    _sync_opened_key(rec, day, kind)


def _sync_opened_key(rec: dict[str, Any], day: str, kind: CheckKind) -> None:
    bucket = ops_day.get_day_bucket(rec, day)
...
def _entry_complete(entry: dict[str, Any] | None) -> bool:
    ...
    return status in ("ok", "legacy", "issue")


def _entry_mark(entry: dict[str, Any] | None) -> str:
    if not entry or not _entry_complete(entry):
        return "⬜"
    return "✅"
...
def shift_gate_complete(rec: dict[str, Any], day: str, kind: CheckKind) -> bool:
    ...
    if not items:
        return True
    return all_checked(rec, day, kind) and remark_step_done(rec, day, kind)
...
def format_config_message(
    ...
    lines = [
        ...
        "",
        "<i>Менеджер отмечает пункты по списку; в конце — что не так и как решили.</i>",
        "",
    ]
    if items:
...
def format_checklist_prompt(
    ...
    if done < len(items):
        lines.append("\n<i>Нажимайте пункты по очереди — галочка ставится сразу.</i>")
    elif not remark_step_done(rec, day, kind):
        lines.append(
            "\n<i>Все пункты отмечены. Напишите замечание или нажмите «Без замечаний».</i>"
        )
    else:
        remark = get_remark(rec, day, kind)
        text = str(remark.get("text") or "").strip() if remark else ""
        if text:
            lines.append(f"\n⚠️ <i>{escape(text)}</i>")
    return "\n".join(lines)
...
def run_checklist_keyboard(rec: dict[str, Any], chat_id: int, day: str, kind: CheckKind):
    ...
    if all_checked(rec, day, kind) and not remark_step_done(rec, day, kind):
        rows.append(
            [
                InlineKeyboardButton(
                    text="📝 Замечание",
                    callback_data=f"cl:rmk:{k}:{cid}:{day}"[:64],
                ),
                InlineKeyboardButton(
                    text="⏭ Без замечаний",
                    callback_data=f"cl:rskip:{k}:{cid}:{day}"[:64],
                ),
            ]
        )
    elif shift_gate_complete(rec, day, kind):
        if kind in ("opening", "opening_waiters"):
            rows.append(
                [
                    InlineKeyboardButton(
...
def collect_checklist_issues(
    ...
    """Замечания по чек-листам за период (итог в конце + старые по пунктам)."""
    ...
    while cur <= end_d:
        ...
        if isinstance(bucket, dict):
            for kind in _CHECKLIST_REPORT_KINDS:
                remark = bucket.get(_remark_key(kind))
                if isinstance(remark, dict) and not remark.get("skipped"):
                    note = str(remark.get("text") or "").strip()
                    if len(note) >= 3:
                        out.append(
                            {
                                "day": day,
