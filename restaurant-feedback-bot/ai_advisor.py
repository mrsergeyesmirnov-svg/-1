...
SYSTEM_PROMPT = """\
Ты — операционный наставник управляющего ресторана. Пишешь взрослому управу:\
 коротко, жёстко по смыслу, без воды и без нравоучений.

Данные анонимны. Погрузись в отзывы по ЗАДАННОЙ теме и выдай рабочую инструкцию\
 именно под эту ситуацию. Не уходи в другие темы.

НЕ делай шаблон:
- не вставляй тет-а-тет и список вопросов «на автомате»;
- вопросы — ТОЛЬКО если без разговора один на один реально не сдвинуть эту тему\
 (например давление в команде / состояние человека). Если тема кухня/процессы/гости —\
 чаще нужна операционная инструкция, а не вопросы;
- не пиши общие фразы вроде «найдите дыру в системе» без конкретного «что именно».

Верни ТОЛЬКО JSON без markdown:
{
  "signal": "что именно повторяется — по сути цитат/отметок, 1-2 предложения",
  "quotes": ["только реальные короткие цитаты из данных"],
  "risk": "что будет при бездействии: конкретно и жёстко (текучка, отзывы гостей, развал смены, потеря людей). Дай почувствовать: откладывать опасно",
  "playbook": [
    "шаг сегодня: максимально конкретно (кто, что, где на смене)",
    "шаг на этой неделе: регулярный менеджмент (что держать каждый день/смену)",
    "как проверить через 3-5 дней, что сдвинулось",
    "что будет, если снова «отпустить» — зачем нужен ритм управления"
  ],
  "questions": [],
  "cta": "один конкретный шаг на ближайшие 24 часа"
}

Правила:
- playbook — сердце ответа: рабочие шаги, не лозунги. Напомни, что без регулярного\
 менеджмента (дисциплина смен, разбор сигналов, контроль) ничего не удержится.
- questions: массив 0–3 строк. Пустой [], если вопросы не нужны.
- quotes: не выдумывай. risk: без морали, но с ценой бездействия.
...
def render_advice_report(
    ...
    *,
    ...
    """Telegram-отчёт: повтор / цитаты / цена бездействия / рабочая инструкция."""
    ...
    # playbook — основной блок; actions — legacy из старых ответов
    playbook = data.get("playbook") or data.get("actions") or []
    ...
    if not isinstance(questions, list):
        questions = []
    # отфильтровать пустые вопросы — не показывать блок зря
    ...
    if quotes:
        ...
        lines.append("")
    if risk:
        lines.append("⚠️ <b>Цена бездействия</b>")
        lines.append(escape(risk))
        lines.append("")
    if playbook:
        lines.append("🛠 <b>Рабочая инструкция</b>")
        for i, step in enumerate(playbook[:5], 1):
            step = str(step).strip()
            if step:
                lines.append(f"{i}. {escape(step)}")
        lines.append("")
    if questions:
        lines.append("🗣 <b>Если нужен разговор</b>")
...
def template_report_data(
    ...
    # Вопросы только для «человеческих» тем; кухня/процессы — операционка
    questions: list[str] = []
    if code in ("team", "conflict", "self", "stress", "staff"):
        for cl in KEYWORD_CLUSTERS:
            if cl["theme_code"] == code:
                questions = list(cl.get("questions") or ())[:2]
                break
    playbook = [
        str(alert.recommendation),
        (
            "Встройте тему в ритм смены: каждый день короткий разбор сигнала "
            "на планёрке без имён — иначе через неделю снова «как будто ничего не было»."
        ),
        (
            "Через 3–5 дней сверьте: стало ли меньше тех же кнопок/фраз в опросе. "
            "Если нет — шаг был формальным, усильте контроль на смене."
        ),
        (
            "Без регулярного менеджмента (кто смотрит сигналы, кто закрывает тему) "
            "даже хороший разовый разговор не удержит результат."
        ),
    ]
    return {
        ...
        "quotes": quotes,
        "risk": (
            "Если не трогать: линия перестанет писать правду, текучка и срывы сервиса "
            "ударят уже по гостю и выручке — откат потом дороже, чем разбор сейчас."
        ),
        "playbook": playbook,
        "questions": questions,
        "cta": "Сегодня закройте один конкретный шаг по этой теме и назначьте, кто держит контроль на ближайших сменах.",
    }
...
async def build_advice(
    ...
    *,
    ...
    if extra_context:
        context += f"\n\nДополнительно:\n{extra_context}"
    context += (
        f"\n\nЖЁСТКО: тема = «{theme_label(theme)}» (code={theme or '-'}). "
        "Не пиши про другие темы.\n"
        "Погрузись в цитаты и выдай рабочую инструкцию (playbook), не шаблон. "
        "questions — только если реально нужны, иначе []. "
        "В risk — цена бездействия, чтобы было ясно: откладывать опасно. "
        "Напомни про регулярный менеджмент, иначе разовое действие не удержит результат."
    )
    try:
        resp = await client.chat.completions.create(
            model=OPENAI_MODEL,
