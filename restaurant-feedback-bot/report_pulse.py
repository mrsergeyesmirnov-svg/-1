...
def _label_for_code(code: str | None, event_type: str) -> str | None:
    if not code:
        return None
...
def _collect_comments(
    ...
    *,
    ...
    for e in picked:
        ...
        plab = _label_for_code(e.problem_code, e.event_type)
...
def _actionable_bullets(
    ...
    for code, n in personal[:2]:
        label = PERSONAL_FACTOR_LABELS.get(code, code)
        tips.append(f"Личный фактор: «{label}» — {n} отметок.")
...
def build_report_html(
    *,
    ...
    comments = _collect_comments(current, period, limit=comment_limit)
    tips = _actionable_bullets(problems, personal, comments)
