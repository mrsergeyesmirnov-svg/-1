...
DAY_INSIGHT_HINTS: dict[str, str] = {
    "kitchen": "Согласуйте с шефом загрузку кухни и приоритеты подачи.",
    "conflict": "Разберите напряжение в команде — это бьёт по сервису и выручке.",
    "staff": "Пересмотрите расстановку и численность на смене.",
    "management": "Уточните роли и процессы — хаос в организации виден линии.",
    "stress": "Снизьте нагрузку или перераспределите задачи на пике.",
    "knowledge": "Добавьте короткий брифинг по процессам перед сменой.",
    "fatigue": "Следите за переработками и ротацией тяжёлых смен.",
    "time_mgmt": "Помогите линии с таймингом: приоритеты столов и пречек.",
    "communication": "Проведите разбор коммуникации зал–кухня–менеджер.",
    "concentration": "Уберите лишние отвлечения в пиковые часы.",
}
...
def _event_dedupe_key(row: EventRow) -> tuple:
    text = _normalize_comment_text(row.comment_text or "")
    if text and row.event_type in (EVENT_COMMENT, "chef_shift"):
        return (
            "comment_body",
...
def _comment_rows(events: list[EventRow]) -> list[EventRow]:
    ...
    out: list[EventRow] = []
    for e in events:
        text = _normalize_comment_text(e.comment_text or "")
        if not text:
            continue
        key = (e.restaurant_chat_id, text[:500])
        if key in seen:
            continue
        seen.add(key)
        out.append(e)
...
def build_report_html(
    *,
    ...
    checklist_issues_line: str | None = None,
    shift_summaries_line: str | None = None,
    ...
    if stop_list_line:
        ...
        lines.append("")
    if checklist_issues_line:
        lines.append(checklist_issues_line)
        lines.append("")
    if shift_summaries_line:
        lines.append(shift_summaries_line)
        lines.append("")
    ...
    *,
    ...
    if not multi:
        ...
        if dept == chef_survey.DEPARTMENT_ALL:
            ...
            issues = shift_checklists.collect_checklist_issues(
                rec if isinstance(rec, dict) else {}, start, end, tz_loc
            )
            checklist_line = shift_checklists.format_checklist_issues_block(issues)
        shift_line_block = None
        if dept != chef_survey.DEPARTMENT_KITCHEN and isinstance(rec, dict):
            summaries = ops_day.collect_shift_summaries(rec, start, end, tz_loc)
            shift_line_block = ops_day.format_shift_summaries_block(summaries)
        cur_loc = [
            e
            for e in current
        ...
        report_msgs = build_report_html(
            ...
            checklist_issues_line=checklist_line,
            shift_summaries_line=shift_line_block,
    ...
    for cid, title in scope:
        ...
        if dept == chef_survey.DEPARTMENT_ALL:
            ...
            issues = shift_checklists.collect_checklist_issues(
                rec_m if isinstance(rec_m, dict) else {}, start, end, tz_loc_m
            )
            checklist_line = shift_checklists.format_checklist_issues_block(issues)
        shift_line_block = None
        if dept != chef_survey.DEPARTMENT_KITCHEN and isinstance(rec_m, dict):
            summaries = ops_day.collect_shift_summaries(
                rec_m, start, end, tz_loc_m
            )
            shift_line_block = ops_day.format_shift_summaries_block(summaries)
        report_msgs = build_report_html(
            period=period,
...
def _events_on_local_day(
    events: list[EventRow], day_key: str, tz_name: str
) -> list[EventRow]:
    try:
        tz = ZoneInfo(tz_name)
    except Exception:
    ...
    for e in events:
        try:
            if e.created_at.astimezone(tz).date() == target:
                out.append(e)
        except Exception:
            continue
    return out
...
def _day_team_snapshot(
    ...
    *,
    ...
    day_ev = _events_on_local_day(events, day_key, tz_name)
    ratings = [e.rating for e in day_ev if e.event_type == EVENT_RATING and e.rating]
    problems: Counter[str] = Counter()
    personal: Counter[str] = Counter()
    comments: list[str] = []
    for e in day_ev:
        if e.event_type == EVENT_PROBLEM and e.problem_code:
            lbl = _label_for_code(
                e.problem_code, EVENT_PROBLEM, problem_labels=problem_labels
            )
            if lbl:
                problems[lbl] += 1
        elif e.event_type == EVENT_PERSONAL and e.problem_code:
            lbl = _label_for_code(
                e.problem_code, EVENT_PERSONAL, problem_labels=problem_labels
            )
            if lbl:
                personal[lbl] += 1
        elif e.event_type == EVENT_COMMENT and (e.comment_text or "").strip():
            comments.append(e.comment_text.strip())
    avg = round(sum(ratings) / len(ratings), 2) if ratings else None
    return {
        "n_raters": len(ratings),
        "avg_rating": avg,
        "top_problems": problems.most_common(3),
        "top_personal": personal.most_common(2),
        "comments": comments[:2],
    }


def _recommendations_from_snapshot(snap: dict[str, Any], *, weak_day: bool) -> list[str]:
    hints: list[str] = []
    code_map = {v: k for k, v in PROBLEM_LABELS.items()}
    code_map.update({v: k for k, v in PERSONAL_FACTOR_LABELS.items()})
    for lbl, _n in snap.get("top_problems", []) + snap.get("top_personal", []):
        code = code_map.get(lbl)
        if code and code in DAY_INSIGHT_HINTS:
            line = DAY_INSIGHT_HINTS[code]
            if line not in hints:
                hints.append(line)
    if weak_day:
        if snap.get("n_raters", 0) < 2:
            hints.append(
                "Мало откликов линии — в этот день команда почти не делилась обратной связью."
            )
        if snap.get("avg_rating") is not None and snap["avg_rating"] < 3.5:
            hints.append(
                "Низкая оценка смены — разберите смену с линией: что мешало работать."
            )
    else:
        if snap.get("avg_rating") is not None and snap["avg_rating"] >= 4.2:
            hints.append(
                "Высокая оценка линии — закрепите удачные практики этого дня."
            )
    return hints[:3]


def build_month_day_insights_html(
    stats: dict[str, Any],
    events: list[EventRow],
    tz_name: str,
    *,
    ...
    if not best and not worst:
        return ""
    lines = ["<b>📅 Лучшие и слабые дни — выводы</b>"]
    pairs: list[tuple[str, tuple[str, float] | None, bool]] = [
        ("Лучший", best, False),
        ("Слабый", worst, True),
    ]
    seen_days: set[str] = set()
    for tag, tup, weak in pairs:
        if not tup:
            continue
        day_key, pct = tup
        ...
        try:
            ...
            day_human = d.strftime("%d.%m")
        except ValueError:
            day_human = day_key
        snap = _day_team_snapshot(
            events, day_key, tz_name, problem_labels=problem_labels
        )
        lines.append("")
        lines.append(f"<b>{tag} день:</b> {escape(day_human)} · план <b>{pct:.0f}%</b>")
        if snap["n_raters"]:
            avg_s = (
                f"{snap['avg_rating']:.2f}".replace(".", ",")
                if snap["avg_rating"] is not None
                else "—"
            )
            lines.append(
                f"Линия: <b>{snap['n_raters']}</b> оценок · средняя <b>{avg_s}</b> ⭐"
            )
        else:
            lines.append("<i>Откликов линии в этот день нет.</i>")
        factors: list[str] = []
        for lbl, n in snap.get("top_problems", []):
            factors.append(f"{lbl} ({n})")
        for lbl, n in snap.get("top_personal", []):
            factors.append(f"{lbl} ({n})")
        if factors:
            lines.append("Что отмечала линия: " + escape(", ".join(factors)))
        for c in snap.get("comments", []):
            short = c[:120] + ("…" if len(c) > 120 else "")
            lines.append(f"<i>«{escape(short)}»</i>")
        if weak and isinstance(rec, dict):
            import ops_day

            evening = ops_day.get_evening(rec, day_key)
            mgr_note = ops_day.shift_summary_text(evening)
            if mgr_note:
                short_m = mgr_note[:200] + ("…" if len(mgr_note) > 200 else "")
                lines.append(
                    f"<b>Менеджер о смене:</b> <i>«{escape(short_m)}»</i>"
                )
        recs = _recommendations_from_snapshot(snap, weak_day=weak)
        if recs:
            lines.append("<b>Как исправить / закрепить:</b>")
            for r in recs:
...
async def build_calendar_month_reports(
    ...
    *,
    ...
    day_insights = build_month_day_insights_html(
        stats, cur, tz, problem_labels=problem_labels, rec=rec if isinstance(rec, dict) else None
    )
    ...
    checklist_line = shift_checklists.format_checklist_issues_block(
        shift_checklists.collect_checklist_issues(
            rec if isinstance(rec, dict) else {}, start, end, tz_loc
        )
    )
    shift_line_block = None
    if isinstance(rec, dict):
        summaries = ops_day.collect_shift_summaries(rec, start, end, tz_loc)
        shift_line_block = ops_day.format_shift_summaries_block(summaries, limit=6)
