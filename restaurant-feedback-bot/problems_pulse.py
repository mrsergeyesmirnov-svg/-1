...
async def sync_problems_from_period(
    ...
    *,
    ...
    existing_rows = await list_problems_for_chat(data, chat_id)
    existing_keys = {p.problem_key for p in existing_rows}
    all_keys = set(thresholds.keys()) | existing_keys
...
def format_problem_list(
    ...
    *,
    ...
    if not rows:
        return (
            f"<b>{header}</b>\n\n"
            f"За последние <b>{sync_days} дн.</b> нет тем, которые набрали порог отметок в опросе.\n"
            "Нажмите «Обновить из отзывов» после накопления откликов."
        )
...
def format_manager_problems_report(
    ...
    *,
    ...
    lines.append("")
    lines.append(
        "Кнопки ниже — открыть тему и сменить статус (в том числе старые). "
