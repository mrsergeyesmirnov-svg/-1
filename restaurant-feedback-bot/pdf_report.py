...
LINE = (231, 226, 220)
BG_WARM = (250, 247, 243)
INSIGHT_BG = (232, 245, 243)
INSIGHT_BORDER = (13, 148, 136)
WHITE = (255, 255, 255)
...
class PulseReportPDF:
    ...
    def _insight_card_height(self, title: str, body_lines: list[str]) -> float:
        pad = 8
        lh_title = 5.5
        lh_body = 5
        h = pad + max(1, len(_wrap_chunks(title, max_len=88))) * lh_title
        for ln in body_lines:
            ...
            h += max(1, len(_wrap_chunks(ln, max_len=88))) * lh_body
        return h + pad
    ...
    def draw_insight_card(self, title: str, body_lines: list[str]) -> None:
        ...
        text_w = self.epw - 12
        start_y = y
        card_h = self._insight_card_height(title, body_lines)

        # Фон и рамка — до текста, иначе текст перекрывается
        p.set_fill_color(*INSIGHT_BG)
        p.set_draw_color(*INSIGHT_BORDER)
        p.rect(p.l_margin, start_y, self.epw, card_h, style="DF")
        p.set_fill_color(*INSIGHT_BORDER)
        p.rect(p.l_margin, start_y, 3, card_h, style="F")

        p.set_xy(text_x, start_y + pad)
        p.set_font(self._font, size=10)
        self._set_color(INK)
        p.multi_cell(text_w, 5.5, title)
        for ln in body_lines:
            p.set_x(text_x)
            low = ln.lower()
            if low.startswith("рекомендация:"):
                p.set_font(self._font, size=9)
                self._set_color(INSIGHT_BORDER)
                p.multi_cell(text_w, 5, "Рекомендация")
                ln = ln.split(":", 1)[-1].strip()
                p.set_x(text_x)
                p.set_font(self._font, size=9.5)
                self._set_color(INK)
                p.multi_cell(text_w, 5, ln)
                continue
            if low.startswith("факторы:"):
                p.set_font(self._font, size=9)
                self._set_color(MUTED)
                p.multi_cell(text_w, 5, "Факторы")
                continue
            p.set_font(self._font, size=9.5)
            self._set_color(INK)
            p.multi_cell(text_w, 5, ln)

        end_y = max(p.get_y() + pad, start_y + card_h)
        if end_y > start_y + card_h:
            extra = end_y - (start_y + card_h)
            p.set_fill_color(*INSIGHT_BG)
            p.rect(p.l_margin, start_y + card_h, self.epw, extra, style="F")
            p.set_fill_color(*INSIGHT_BORDER)
            p.rect(p.l_margin, start_y, 3, end_y - start_y, style="F")
            p.set_draw_color(*INSIGHT_BORDER)
